...
export default async function SearchPage({
  ...
  const [bookData, users, authorData, tagKeys, statusMap, popularBooks, popularAuthors, popularUsers, savedSearches, trendingBooks] = await Promise.all([
                    ...
                    <li key={author.name} className="flex items-center gap-3 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-text-secondary mt-0.5">
                          {author.book_count} book{author.book_count === 1 ? "" : "s"} on Rosslib
                        </p>
                      </div>
                    </li>
                  ))}
