...
500 { "error": "Failed to save genre ratings" }  (deleting stale ratings failed)
...
Returns series containing books whose `authors` field matches the given name. The `name` query param is required and must be 500 characters or fewer (`400` otherwise). Capped at 50 results.
...
`rating` is 1–5 (0 clears). Out-of-range values return `400 { "error": "Rating must be between 1 and 5" }`. `review_text` longer than 10,000 characters returns `400 { "error": "Review must be 10,000 characters or fewer" }`.
...
Creates a recommendation record and sends a `book_recommendation` notification to the recipient. Also records a `sent_recommendation` activity. Returns `201` with the recommendation ID. Returns `409` if the same sender/recipient/book triple already exists. Returns `404 "User not found"` when a block exists in either direction between sender and recipient (deliberately indistinguishable from a nonexistent user). Rate-limited to 10 recommendations per 24-hour window — returns `429` with `"too many recommendations, try again later"` if exceeded.
