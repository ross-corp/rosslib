...
Per-user notification rows. Currently only `new_publication` type exists (generated by the author works poller), but the schema is generic for future notification types.
