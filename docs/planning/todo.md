...
- [ ] Fix N+1 query in `GetBookThreads` (`api/handlers/threads.go` ~line 68): the handler loops over each thread and runs a separate `SELECT COUNT(*)` query for comment_count. Refactor to use a single query with `LEFT JOIN thread_comments` and `GROUP BY` to batch-fetch comment counts, or use a subquery in the initial thread listing query.
