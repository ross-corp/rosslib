...
- [ ] Smart shelf mapping predictions for Goodreads import. Frontend-only change in `webapp/src/components/import-form.tsx`. Add a `predictShelfMappings(shelves: string[])` helper that auto-detects label groupings: (1) **Prefix detection** — split shelf names on `-` or `_`, group by first segment, if 2+ shelves share a prefix predict them as `create_label` with the prefix in title case (e.g. `genre-scifi`, `genre-fantasy` → "Genre"). (2) **Year detection** — match `/^(?:.*[-_])?((?:19|20)\d{2})$/`, if 2+ year-like shelves found predict them as `create_label` with "Year Read". Year overrides prefix. Call predictions in `enterConfigure()` as fallback defaults: `shelfMappings.get(shelf) ?? predictions.get(shelf) ?? { action: "tag" }`. Redesign the action `<select>` to use compound values merging action + label name shortcuts: options are `tag`, `skip`, `create_label:new` (shows text input), `create_label:{name}` (reuse existing session label, no text input), `existing_label:{id}`. Derive `sessionLabelNames` from current mappings so newly created label names appear as shortcuts on other rows. Only show the label name text input when compound value is `create_label:new`.
...
- [Add edition selector to book detail page](https://github.com/ross-corp/rosslib/pull/40) — Users can select a specific edition cover; displayed across profile, shelf, and tag pages
