...
- [ ] Add `error.tsx` boundary for the feed page: create `webapp/src/app/feed/error.tsx` that catches server errors from `GET /me/feed` and shows a "Something went wrong" message with a "Try again" button (use `reset()` from the error boundary props). Follow the pattern in `webapp/src/app/books/[workId]/error.tsx`.
